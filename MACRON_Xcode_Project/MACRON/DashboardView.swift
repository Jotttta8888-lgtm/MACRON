            ...
            VStack(spacing: 24) {
                ...
                if let hardware = api.status?.hardware {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 200))], spacing: 16) {
                        StatCard(title: "Apple Silicon", value: hardware.appleSilicon ? "M-Series" : "Intel", icon: "cpu", color: hardware.appleSilicon ? .cyan : .gray)
                        StatCard(title: "MLX", value: hardware.mlx ? "Activo" : "Inactivo", icon: "bolt.fill", color: hardware.mlx ? .green : .gray)
                        StatCard(title: "RAM", value: "\(Int(hardware.ramGb)) GB", icon: "memorychip", color: .orange)
                        StatCard(title: "Modelo LLM", value: hardware.model, icon: "brain", color: .purple)
                    }.padding(.horizontal)
                }
