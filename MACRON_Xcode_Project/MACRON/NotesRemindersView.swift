...
struct AddNoteSheet: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            TextField("Tags (separados por coma)", text: $tags).textFieldStyle(.roundedBorder)
            HStack {
...
struct ReminderRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: reminder.completed ? "checkmark.circle.fill" : "circle").foregroundColor(reminder.completed ? .green : .secondary).font(.title3)
            VStack(alignment: .leading, spacing: 2) {
                Text(reminder.title).font(.body).strikethrough(reminder.completed)
                HStack(spacing: 8) {
