    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 12) {
                ...
                TextField("Buscar en la web...", text: $query).textFieldStyle(.plain).onSubmit { search() }
...
struct ResearchResultRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(result.title).font(.headline).foregroundColor(.accentColor)
...
struct AnalyticsView: View {
                                        ...
                                        HStack {
                                            ...
                                            GeometryReader { geo in
                                                RoundedRectangle(cornerRadius: 4).fill(Color.accentColor)
                                                    .frame(width: CGFloat(day.count) / CGFloat(maxCount(daily)) * (geo.size.width - 40), height: 20)
                                            }
                                            Text("\(day.count)").font(.caption).frame(width: 30, alignment: .trailing)
                                        }
                                    }
                                }
    ...
    private func maxCount(_ usage: [DailyUsage]) -> Int { usage.map(\.count).max() ?? 1 }
}
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Form {
                ...
                Section("Conexion") {
                    HStack { Text("Puerto API"); Spacer(); TextField("Puerto", text: $selectedPort).textFieldStyle(.roundedBorder).frame(width: 80) }
