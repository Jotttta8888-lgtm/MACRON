    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 12) {
                ...
                NativeTextField(text: $query, placeholder: "Buscar archivos...", onSubmit: { search() })
                NativeTextField(text: $path, placeholder: "Ruta", onSubmit: nil).frame(width: 150)
...
struct FinderItemRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: item.icon ?? "doc").font(.title3).foregroundColor(.accentColor).frame(width: 30)
            VStack(alignment: .leading, spacing: 2) {
                Text(item.name ?? "Sin nombre").font(.body)
            ...
            Spacer()
            Button(action: { if let p = item.path { NSWorkspace.shared.selectFile(p, inFileViewerRootedAtPath: "") } }) {
...
struct SafariView: View {
    ...
    var body: some View {
        ...
        .background(Color(.windowBackgroundColor))
        .task { await loadData() }
        .onChange(of: selectedTab) { oldValue, newValue in Task { await loadData() } }
    }
...
struct BookmarkRow: View {
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(bookmark.title ?? "Sin titulo").font(.body)
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(message.subject ?? "Sin asunto").font(.body).fontWeight(.medium)
