...
def chat():
    ...
    print(f"[SERVER] Received message: {message[:50]}")
    try:
        result = macron.llm.chat(message)
        print(f"[SERVER] result type: {type(result)}")
        print(f"[SERVER] result: {repr(result)[:200]}")

        text = ""
        if isinstance(result, dict):
            text = result.get('text', '') or result.get('response', '') or result.get('content', '') or str(result)
        elif isinstance(result, str):
            text = result
        else:
            text = str(result)

        print(f"[SERVER] extracted text: {text[:100]}")
